...
export class LivreUpdatePage {
  ...
  desciptionInput = element(by.id('field_desciption'));
  ...
  async setDesciptionInput(desciption: string): Promise<void> {
    await this.desciptionInput.sendKeys(desciption);
  }

  async getDesciptionInput(): Promise<string> {
    return await this.desciptionInput.getAttribute('value');
  }
